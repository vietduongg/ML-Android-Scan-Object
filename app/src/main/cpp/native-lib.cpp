...
Java_com_vietduong_HiU_MainActivity_stringFromJNI(
        JNIEnv* env,
        jobject ) {
...
Java_com_vietduong_HiU_ObjectDetection_initDetector(JNIEnv* env, jobject p_this, jobject assetManager) {
    ...
    if (!(env->IsSameObject(assetManager, NULL))) {
        ...
        AAsset *asset = AAssetManager_open(mgr, "od_model.HiU", AASSET_MODE_UNKNOWN);
...
Java_com_vietduong_HiU_ObjectDetection_destroyDetector(JNIEnv* env, jobject p_this, jlong p_native_ptr) {
    if (p_native_ptr)
        delete (ObjectDetector *) p_native_ptr;
}

extern "C" JNIEXPORT jfloatArray JNICALL
Java_com_vietduong_HiU_ObjectDetection_detect(JNIEnv* env, jobject p_this,
